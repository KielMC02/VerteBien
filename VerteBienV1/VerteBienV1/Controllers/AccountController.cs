...
namespace VerteBienV1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    //Validando si el email del usuario esta confirmado antes de permitirle acceder.
                    ...
                    if (!UserManager.IsEmailConfirmed(userid)) 
                    {
                        ...
                        ModelState.AddModelError("", "Debes confirmar tu cuenta, revisa tu correo electronico");
                        return View(model);
                    }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, string membresiaSelec)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, nombre = model.nombre, apellido = model.apellido, fecha_nacimiento_ = model.fecha_nacimiento_, fecha_creacion_ = DateTime.Today, ciudad = model.ciudad, sector = model.sector, calle = model.calle, telefono = model.telefono,  latitud = model.latitud,nombre_peluqueria = model.nombre_peluqueria, longitud = model.longitud, estado = model.estado };
                if (membresiaSelec == "expres")
                {
                    user.capacidad_simultanea_ = 2;
                }
                if (membresiaSelec == "preferencial")
                {
                    user.capacidad_simultanea_ = 6;
                }
        ...
        // GET: /Account/Registro de Usuario Cliente
        [AllowAnonymous]
        ...
        public async Task<ActionResult> RegisterUser(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //Asignamos Membresia al usuario.
                    var AgregarMembresia = UserManager.AddToRole(IdUsuarioCreado, "cliente");
                    //await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
