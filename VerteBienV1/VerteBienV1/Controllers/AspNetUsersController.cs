...
namespace VerteBienV1.Controllers
{
    ...
    public class AspNetUsersController : Controller
    {
        ...
        // GET: AspNetUsers
        public ActionResult Index(string email, string estatusUserSelec)
        {
            List<AspNetUsers> usuarios = new List<AspNetUsers> ();

            if(email != null && estatusUserSelec != null && email != "" && estatusUserSelec != "")
            {
                usuarios = (from busqueda in db.AspNetUsers where busqueda.Email.Contains(email) && busqueda.estado.Contains(estatusUserSelec) select busqueda).ToList();
                return View(usuarios);
            }

            if (email != null && email != "" && (estatusUserSelec == null || estatusUserSelec =="" ))
            {
                usuarios = (from busqueda in db.AspNetUsers where busqueda.Email.Contains(email) select busqueda).ToList();
                return View(usuarios);
            }
            if (estatusUserSelec != null && estatusUserSelec != "")
            {
                usuarios = (from busqueda in db.AspNetUsers where busqueda.estado.Contains(estatusUserSelec) select busqueda).ToList();
                return View(usuarios);
            }
        ...
        public ActionResult Edit(string Id)
        {
            ...
            if (User.IsInRole("administrador") && Id != null)
            {
                AspNetUsers aspNetUsers = db.AspNetUsers.Find(Id);
                //Listado de sectores
                #region
